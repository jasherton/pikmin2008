...
function SWEP:Holster()
	if (SERVER) then
		...
		if (ValidEntity(throwpikmin)) then
			throwpikmin = nil;
		end
...
local function PikSWepKeyPress(ply, key)
	if (SERVER) then
		if (ply:Alive()) then
			if (ValidEntity(ply:GetActiveWeapon()) && ply:GetActiveWeapon():GetClass() == "olimar_gun") then
				...
				if (key == IN_RELOAD) then
					if (!ply:KeyDown(IN_USE)) then
						...
						local pikList = {}
						local pikRed,pikYellow,pikBlue = false,false,false
						local pikTypes = 0
						for k,v in ipairs(ents.FindByClass("pikmin")) do
							if (v.Olimar == ply) then
								doanim = true;
								table.insert(pikList,v)
								centerPos = centerPos + v:GetPos()
								if (v:GetPikType() == "red" && !pikRed) then
									pikRed = true
									pikTypes = pikTypes + 1
								elseif v:GetPikType() == "yellow" && !pikYellow then
									pikYellow = true
									pikTypes = pikTypes + 1
								elseif v:GetPikType() == "blue" && !pikBlue then
									pikBlue = true
									pikTypes = pikTypes + 1
								end
							end
						end
						centerPos = centerPos / #pikList
						for k, v in ipairs(pikList) do
							if pikTypes > 1 then
								if v:GetPikType() == "red" then
									v:Disband(centerPos+Vector(50,-50,0));
								elseif v:GetPikType() == "yellow" then
									v:Disband(centerPos+Vector(0,50,0));
								elseif v:GetPikType() == "blue" then
									v:Disband(centerPos+Vector(-50,-50,0));
								end
							else
								v:Disband()
...
local function PikSWepKeyRelease(ply, key) //release primary, throw pikmin
	if (SERVER) then
		if (ply:Alive()) then
			if (ValidEntity(ply:GetActiveWeapon()) && ply:GetActiveWeapon():GetClass() == "olimar_gun") then
				if (key == IN_ATTACK) then
					if (ValidEntity(throwpikmin) && ply.HasPikmin) then
						timer.Destroy("PikPrimaryIdleAnim" .. ply:UniqueID());
						timer.Destroy("PikDeployIdleAnim" .. ply:UniqueID());
	...
	if (ValidEntity(tr.Entity)) then
		for k, v in pairs(ents.FindByClass("pikmin")) do
			if (v.Olimar == self.Owner && (tr.Entity:IsNPC() || tr.Entity:IsPlayer())) then
				...
				timer.Create("Pik2ndryIdleAnim" .. self.Owner:UniqueID(), 1.3, 1, self.SendWeaponAnim, self, ACT_VM_IDLE);
