...
local function PikminCreate(ply, cmd, args)
	...
	if (args[1] == "red" || args[1] == "yellow" || args[1] == "blue" || args[1] == "random") then
		...
		ply:ChatPrint("Not a valid color: red, yellow, blue");
	end
end
...
function ENT:Disband(pos)
	if (!self.Attacking && !ValidEntity(self.AtkTarget)) then //Don't disband if we're attacking or charging after an enemy, our leader still wants the kill!
		...
		if pos and self:GetVelocity():Length() <= 50 then
			self.DismissTimer = CurTime() + 2
			self.DismissShadowAng = Angle(0,0,0);
			self.DismissPos = pos
		end
		self.Olimar = nil;
		self.Dismissed = true;
...
function ENT:Think()
	...
	if (vel:Length() >= 6 && !self.Victim && (!self.ShouldSpin || !self.JustThrown) && !self.RejoinAnim && !self.Sucking) then
	...
	if (self.CanMove) then
		local opos = pos;
		local dist;
		if (self.Dismissed and self.DismissPos) then
			opos = self.DismissPos;
			dist = 300;
			if (CurTime() >= self.DismissTimer || pos:Distance(opos) <= 10) then
				self.DismissPos = nil;
			end
		end
		if (!self.Dismissed && ValidEntity(self.Olimar)) then
			...
			if (CurTime() >= self.LastHop) then
				...
				if (dist >= 200) then
					if (vel:Length() <= 250) then
						...
						if self.Dismissed then force = 500 zforce = 125 end
